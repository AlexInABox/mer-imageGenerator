...
public class CubeMosaicGenerator : MonoBehaviour
{
    ...
    private void optimizeHorizontally()
    {
        ...
        */
        ...
        //put all cubes in a list by collumn
        List<List<GameObject>> cubesByCollumn = new List<List<GameObject>>();
        float previousX = -1000000;
        foreach (Transform child in children)
        {
            if (child.localPosition.x != previousX)
            {
                cubesByCollumn.Add(new List<GameObject>());
            }
            cubesByCollumn[cubesByCollumn.Count - 1].Add(child.gameObject);
        ...
        for (int collumn = 0; collumn < cubesByCollumn.Count; collumn++)
        {
            for (int i = 0; i < cubesByCollumn[collumn].Count; i++)
            {

                GameObject currentCube;
                if (cubesByCollumn[collumn][i])
                {
                    currentCube = cubesByCollumn[collumn][i];
                }
                else
                {
                    continue;
                ...
                for (int a = collumn + 1; a < cubesByCollumn.Count; a++)
                {

                    GameObject validCubeToTheRight = null;

                    foreach (GameObject cubeToTheRight in cubesByCollumn[a])
                    {
                ...
                foreach (GameObject validCubeToTheRight in validCubesToTheRight)
                {
                    ...
                    foreach (List<GameObject> collumnList in cubesByCollumn)
                    {
                        if (collumnList.Contains(validCubeToTheRight))
                        {
                            collumnList.Remove(validCubeToTheRight);
                            break;
                        }
                    }
